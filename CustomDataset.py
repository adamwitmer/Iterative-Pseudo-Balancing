...
class CustomDataset(data.Dataset):
    ...
    def __init__(self, img_root, crop_size, thresh, remove_background, crop_params=None, transform=None, target_transform=None,  # tensor_root
        ...
        self.augmentations = RandAugment(n=2, m=10)
    ...
    def __getitem__(self, index):
        ...
        if self.crop_params is None:
            ...
            img_bin, img_crop, _ = MapCrop(self.crop_size, tensor=tensor_sample, thresh=self.thresh, remove_background=self.remove_background, crop_params=crop_indices)(img)  # Random binarization (0.25)
        ...
        # img = self.augmentations(img_bin)
        ...
        # img2 = self.augmentations(img_bin)
        ...
        return img, img2, img_gray, index, target, crop_indices
...
class CustomDatasetTest(data.Dataset):
    ...
    def __init__(self, img_root, crop_size, thresh, transform=None, target_transform=None,  # tensor_root
                 loader=default_loader, normalize=True):
        classes, class_to_idx = find_classes(img_root)
        imgs = make_dataset(img_root, class_to_idx)
        # tensors = make_dataset(tensor_root, class_to_idx)
